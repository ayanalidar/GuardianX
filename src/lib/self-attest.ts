...
// The middleware imports these names — provide them as aliases to the
// cached verifyIntegrity function + a cache invalidator.

/** Cached integrity verification. Accepts optional {projectRoot} for
 *  middleware compatibility (ignored — we use process.cwd() internally).
 *  On Vercel, source files are bundled differently (no src/ dir at runtime),
 *  so the baseline may be empty. An empty baseline = no files to check =
 *  ok: true (fail-open for serverless compatibility). */
export async function verifyIntegrityCached(_opts?: { projectRoot?: string }): Promise<IntegrityStatus> {
  // If the critical directories don't exist (Vercel bundles differently),
  // ensureBaseline returns an empty map. verifyIntegrity handles this
  // gracefully — empty baseline = ok: true.
  const baseline = ensureBaseline();
  if (baseline.size === 0) {
    return {
      ok: true,
...
/** Invalidate the cached verification result (forces a re-check on next call). */
