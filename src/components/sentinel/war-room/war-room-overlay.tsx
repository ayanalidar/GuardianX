...
export function WarRoomOverlay({ open, onClose, initialView = "overview" }: WarRoomOverlayProps) {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "v" || e.key === "V") setVoiceOn((v) => !v);
      ...
      >
        ...
        <AnimatePresence>
          {voiceOn && (
            ...
            >
              <VoiceControl ref={voiceRef} onCommand={handleVoiceCommand} speakResponses={false} />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="pointer-events-none absolute bottom-1 left-1/2 z-10 -translate-x-1/2">
          <div className="rounded-full border border-zinc-700/50 bg-zinc-950/60 px-4 py-1 font-mono text-[10px] uppercase tracking-widest text-zinc-500 backdrop-blur">
            ESC — exit · ← → — switch view · V — voice · G — gesture · fist — close
          </div>
        </div>
      </motion.div>
    </AnimatePresence>
