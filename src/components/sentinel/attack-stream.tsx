...
import type { RedAgentEvent } from "@/lib/sentinel/api";
import {
  AlertCircle,
  CheckCircle2,
  Info,
...
export function AttackStream({
  ...
  const currentStage = status ?? "queued";
  const findCount = events.filter(
    (e) => e.level === "success" && e.message.includes("FINDING:")
  ).length;
      ...
      <div className="border-b border-zinc-800 p-4">
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="flex size-7 items-center justify-center rounded-md bg-red-500/15 ring-1 ring-red-500/40">
            ...
            {findCount > 0 && (
              <Badge className="border border-red-500/40 bg-red-500/10 text-[10px] text-red-300">
                {findCount} finding{findCount === 1 ? "" : "s"}
              </Badge>
            )}
          </div>
          <div className="flex items-center gap-2">
              ...
              <div className="flex h-64 flex-col items-center justify-center gap-2 text-zinc-600">
                ...
                <p className="text-center text-[11px]">
                  {active
                    ...
                    : "Start an engagement to see live attacks."}
                </p>
              </div>
            ) : (
              <AnimatePresence initial={false}>
                {events.map((e, i) => {
                  const isFinding = e.level === "success" && e.message.includes("FINDING:");
                  return (
                    <motion.div
                      key={`${e.ts}-${i}`}
                    ...
                    >
                      ...
                      <span
                        className={`min-w-0 flex-1 break-words ${
                          isFinding ? "text-red-200" : "text-zinc-300"
                        }`}
                      >
                        {e.message}
                      </span>
