    ...
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <a href="/" className="flex items-center gap-2.5">
          ...
          <Badge className="ml-1 hidden border-emerald-500/30 bg-emerald-500/10 text-[9px] text-emerald-300 sm:inline-block">
            SOC
          </Badge>
        </a>
                  ...
                  <div className="w-72 overflow-hidden rounded-lg border border-emerald-500/20 bg-zinc-950/95 p-2 shadow-2xl backdrop-blur-xl">
                    {item.children.map((child) => (
                      ...
                      >
                        ...
                        {child.desc && (
                          <div className="mt-0.5 text-[11px] text-zinc-500">{child.desc}</div>
                        )}
                      </a>
                    ))}
        ...
        <div className="flex items-center gap-2">
          <a
            href="/contact"
            className="hidden rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-4 py-2 text-sm font-medium text-emerald-300 transition-all hover:bg-emerald-500/20 hover:text-emerald-200 sm:inline-flex"
          >
          ...
          <button
            ...
            className="text-zinc-400 hover:text-emerald-400 lg:hidden"
          ...
          <div className="flex flex-col gap-1 px-4 py-3">
            {NAV_ITEMS.map((item) => (
              <div key={item.href + item.label}>
                ...
                >
                  {item.label}
                  {item.children && <ChevronDown className={`size-3.5 transition-transform ${openDropdown === item.href ? "rotate-180" : ""}`} />}
                </a>
                {/* Mobile sub-items */}
                {item.children && openDropdown === item.href && (
            ...
            <a
              href="/contact"
              onClick={() => setMobileOpen(false)}
              className="mt-2 flex items-center justify-center rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-4 py-2.5 text-sm font-medium text-emerald-300"
            >
